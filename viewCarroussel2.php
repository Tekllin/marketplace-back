...
try {
    ...
    $sql = is_numeric($idPub) ? "SELECT * FROM publicites2 WHERE idPub = '$idPub'" : "SELECT * FROM publicites ORDER BY RAND()";
    ...
    if($stmt->rowCount() > 0) {
        ...
        $sql = "SELECT idImagePub, libelleImagePub, idPubliciteImage FROM imagepub";
        $stmt = $conn->prepare($sql);
