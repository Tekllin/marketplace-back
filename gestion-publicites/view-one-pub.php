...
try {
    ...
    $sql = is_numeric($idPub) ? "SELECT * FROM publicites WHERE idPub = '$idPub'" : "SELECT * FROM publicites";
    ...
    if($stmt->rowCount() > 0) {
        ...
        $sql = "SELECT idImagePub, libelleImagePub, idPubliciteImage FROM imagepub WHERE idPubliciteImage = $idPub";
        $stmt = $conn->prepare($sql);
