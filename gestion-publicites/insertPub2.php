...
if(!isset($libellePub) || !isset($lien)) {
    ...
    try {
        ...
        try {
            $req = $conn->prepare("SELECT max(idPub) as nbPub FROM publicites2");
            $req->execute();
            $data = $req->fetch();
            ...
            for ($i = 0; $i < count($imagePub); $i++) {
                if ($imagePub[$i]['error'] == UPLOAD_ERR_OK) {
                    ...
                    move_uploaded_file($tmp_name, "../photos/pubs2/$name");
                    
                    $req = $conn->prepare("INSERT INTO imagepub2(libelleImagePub, idPubliciteImage) VALUES ('$name','$nb')");
                    $req->execute();
                }
            }
        } catch (Exception $e) {
