<?php
    ...
    try 
    {
        $fetch_post = "SELECT idImg, idPageImg, lienImg FROM imgpages WHERE idPageImg = :idPageImg";
        $fetch_stmt = $cnx->prepare($fetch_post);
        $fetch_stmt->bindValue(':idPImg', $idPageImg, PDO::PARAM_INT);
        $fetch_stmt->execute();

        if ($fetch_stmt->rowCount() > 0) :

            $delete_post = "DELETE FROM `imgpages` WHERE idPagesImg = :idPagesImg";
            $delete_post_stmt = $cnx->prepare($delete_post);
            $delete_post_stmt->bindValue(':idPagesImg', $idPagesImg, PDO::PARAM_INT);
