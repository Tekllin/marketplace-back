<?php
    ...
    try 
    {
        ...
        $fetch_stmt->bindValue(':idCategorie', $idCategorie, PDO::PARAM_INT);
        $fetch_stmt->execute();

        if ($fetch_stmt->rowCount() > 0) :

            $delete_post = "DELETE FROM `cattutoactu` WHERE idCategorie = :idCategorie";
            $delete_post_stmt = $cnx->prepare($delete_post);
            $delete_post_stmt->bindValue(':idCategorie', $idCategorie, PDO::PARAM_INT);
            ...
            echo json_encode(['success' => 0, 'message' => 'Invalid ID. No categorie found by the ID.']);
            exit;
        endif;
